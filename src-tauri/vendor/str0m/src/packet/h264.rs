...
impl H264Packetizer {
    ...
    fn emit(&mut self, nalu: &[u8], mtu: usize, payloads: &mut Vec<Vec<u8>>) {
        ...
        if nalu_type == AUD_NALU_TYPE || nalu_type == FILLER_NALU_TYPE {
            ...
            // screenmirror patch: emit SPS as a single NALU instead of bundling
            // it via STAP-A. RFC 6184 forbids STAP-A when packetization-mode=1,
            // and Chrome silently discards the entire IDR frame as a result.
            // Single-NALU SPS works under both mode=0 and mode=1.
            if nalu.len() <= mtu {
                payloads.push(nalu.to_vec());
            }
            // Note: SPS/PPS are always tiny (a few dozen bytes) so FU-A
            // fragmentation is unreachable in practice. If they ever exceed
            // MTU, the FU-A code path further below would handle them, but
            // we return early to keep behavior identical to the cache step.
            return;
        } else if nalu_type == PPS_NALU_TYPE {
            self.pps_nalu = Some(nalu.to_vec());
            // screenmirror patch: emit PPS as a single NALU (same reason as SPS).
            if nalu.len() <= mtu {
                payloads.push(nalu.to_vec());
            }
            return;
        } else if let (Some(_sps_nalu), Some(_pps_nalu)) = (&self.sps_nalu, &self.pps_nalu) {
            // Pack current NALU with SPS and PPS as STAP-A
            // screenmirror patch: STAP-A emission is DISABLED. RFC 6184
            // forbids STAP-A when packetization-mode=1 is negotiated, and
            // Chrome silently drops the entire IDR frame in that case, so
            // the viewer stays at videoWidth=0. SPS/PPS are sent as their
            // own single-NALU packets above; here we just fall through to
            // the single-NALU / FU-A path for the IDR NALU itself.
        }
